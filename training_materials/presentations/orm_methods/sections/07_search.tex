...
% 7. search() ORM Method
% =========================================================
\section{\texttt{search()} ORM Method}

\begin{frame}[standout]
  \texttt{search()} ORM Method
\end{frame}

\begin{frame}[fragile]{\texttt{SEARCH()} METHOD}
  \begin{itemize}
    \item The \texttt{search()} method is an ORM method used to \textbf{find records}
          that match a domain (filter condition).
  ...
  \begin{block}{Method Syntax}
    The standard syntax of the \texttt{search()} method is:
\begin{lstlisting}
@api.model
def search(self, domain, offset=0, limit=None, order=None):
    return super().search(domain, offset=offset, limit=limit, order=order)
\end{lstlisting}
    Important parts: \texttt{domain}, \texttt{offset}, \texttt{limit}, \texttt{order}.
  \end{block}
\end{frame}

\begin{frame}[fragile]{BREAKING DOWN THE METHOD SIGNATURE}
  \begin{itemize}
    \item \texttt{@api.model}: Search runs at model level (not on one existing record).
    \item \texttt{self}: Represents the model, e.g.\ \texttt{student.student()}.
    \item \texttt{domain}: A list of conditions used to filter records.
  \end{itemize}
\begin{lstlisting}
...
\begin{frame}[fragile]{OFFSET, LIMIT, AND ORDER}
  \begin{itemize}
    \item \texttt{offset}: Skip the first N records (used for pagination).
    \item \texttt{limit}: Return at most N records.
    \item \texttt{order}: SQL-like ordering string.
  \end{itemize}
\begin{lstlisting}
...
\begin{frame}[fragile]{WHAT DOES \texttt{SEARCH()} RETURN?}
  \begin{itemize}
    \item The \texttt{search()} method returns a recordset.
  \end{itemize}
\begin{lstlisting}
students = self.env['student.student'].search([
    ('name', 'ilike', 'ahmed'),
...
\end{lstlisting}
  \begin{itemize}
    \item Here, \texttt{.search(...)} is the method call.
