...
\section{\textbf{\texttt{sorted()}} ORM Method}

\begin{frame}[fragile]{\textbf{\texttt{sorted()}} Method}
	\begin{itemize}
		\item The \textbf{\texttt{sorted()}} method returns a recordset ordered in Python.
...
\end{lstlisting}
	\begin{itemize}
		\item Important parts: \textbf{\texttt{key}}, \textbf{\texttt{reverse}}.
	\end{itemize}
\end{block}
	\end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Breaking Down the Method Signature}
	\begin{block}{}
		\begin{itemize}
			\item \textbf{\texttt{self}}: The recordset to sort.
			\item \textbf{\texttt{key}}: A field name string, or a function that returns the sort value.
			\item \textbf{\texttt{reverse}}: \texttt{False} for ascending, \texttt{True} for descending.
\begin{lstlisting}
...
\end{lstlisting}
	\begin{itemize}
		\item Here, \textbf{\texttt{.sorted(...)}} is the \textbf{method call}.
